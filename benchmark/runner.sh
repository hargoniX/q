...
variant=$1
duration=${2:-"1"}
SELECTION_STRATEGY=${3:-"first-neg"}
REPO_ROOT=`git rev-parse --show-toplevel`

pushd $REPO_ROOT > /dev/null
if [ "${variant}" == "pelletier" ]; then
  python benchmark/runner.py -d $duration -m custom -f benchmark/pelletier.toml -s $SELECTION_STRATEGY
  ...
  python benchmark/runner.py -d $duration -m custom -f benchmark/casc29_ueq.toml -s $SELECTION_STRATEGY
  ...
  python benchmark/runner.py -d $duration -m custom -f benchmark/contradictory.toml -s $SELECTION_STRATEGY
elif [ "${variant}" == "countersat" ]; then
  python benchmark/runner.py -d $duration -m custom -f benchmark/counter_satisfiable.toml -s $SELECTION_STRATEGY
elif [ "${variant}" == "sat" ]; then
  python benchmark/runner.py -d $duration -m custom -f benchmark/satisfiable.toml -s $SELECTION_STRATEGY
elif [ "${variant}" == "theorems" ]; then
  python benchmark/runner.py -d $duration -m custom -f benchmark/theorems.toml -s $SELECTION_STRATEGY
elif [ "${variant}" == "unsat" ]; then
  python benchmark/runner.py -d $duration -m custom -f benchmark/unsat.toml -s $SELECTION_STRATEGY
else
